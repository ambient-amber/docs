...
/*
    ...
    2) Identity map
        ...
        $entity1 === $entity2
        Второго запроса не будет. Данные собираются в массиве с уникальным ключем.
        Если дернуть 1000 строк из бд, то все они появятся в этом массиве, осторожно.

    3) Data mapper
        Данные не напрямую попадают в объект из БД и наоборот, они проходят этап преобразования.
        Например при указании типа поля сущности могут использоваться методы:
...
/*
    ...
    1) Класс сущности ни от чего не наследуется.
    2) Нет магических методов __set, __get
    3) Умеет работать с приватными полями
    3) Используются методы для преобразования данных, полученных из БД или добавляемых в БД
*/
